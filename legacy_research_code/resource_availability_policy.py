...
"""
Resource Availability Policy Mining Framework
...
class ResourceAvailabilityPolicyMining:
    ...
    def __init__(self, log_path: str = None, output_dir: str = 'results'):
        ...
        self.violation_info = {}  # Information about injected violations
    ...
    def augment_event_log_with_violations(self, violation_rate: float = 0.05) -> pd.DataFrame:
        ...
        # Add a column to track if an event is a violation
        augmented_df['is_violation'] = False
        ...
        for idx in violation_indices:
            ...
            augmented_df.at[idx, 'is_violation'] = True
        ...
        print(f"Augmented event log with {num_violations} violations")
    ...
    def evaluate_detection_performance(self) -> Dict[str, Dict[str, float]]:
        ...
        if 'is_violation' not in self.log_df.columns:
            raise ValueError("Event log not augmented with violations.")
    ...
    def visualize_results(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'availability_violation_detection_results.png'), dpi=300)
        print(f"Saved detection results visualization to {os.path.join(self.output_dir, 'availability_violation_detection_results.png')}")
        ...
        # Get top resources by violation count
        resource_violations = pd.DataFrame({
            'Resource': list(self.violation_info['by_resource'].keys()),
            'Violations': list(self.violation_info['by_resource'].values())
        })
        resource_violations = resource_violations.sort_values('Violations', ascending=False)
        ...
        plt.title('Top Resources by Violation Count', fontsize=14)
        plt.xlabel('Resource', fontsize=12)
        plt.ylabel('Number of Violations', fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'violations_by_resource.png'), dpi=300)
        print(f"Saved violations by resource visualization to {os.path.join(self.output_dir, 'violations_by_resource.png')}")
        ...
        with open(os.path.join(self.output_dir, 'availability_summary_results.txt'), 'w') as f:
            f.write('Resource Availability Violation Detection Results Summary\n')
            f.write('====================================================\n\n')
            f.write(f'Total events analyzed: {self.violation_info["total_events"]}\n')
            f.write(f'Total injected violations: {self.violation_info["total_violations"]} ({self.violation_info["violation_rate"]*100:.2f}%)\n\n')
        ...
        print(f"Saved summary results to {os.path.join(self.output_dir, 'availability_summary_results.txt')}")
    ...
    def save_results(self):
        ...
        self.log_df.to_csv(os.path.join(self.output_dir, 'availability_analysis_results.csv'), index=False)
        
        # Save violation information
        with open(os.path.join(self.output_dir, 'violation_info.txt'), 'w') as f:
            f.write('Violation Information\n')
            f.write('====================\n\n')
            f.write(f'Total events: {self.violation_info["total_events"]}\n')
            f.write(f'Total violations: {self.violation_info["total_violations"]}\n')
            f.write(f'Violation rate: {self.violation_info["violation_rate"]*100:.2f}%\n\n')
            
            f.write('Violations by Resource:\n')
            for resource, count in self.violation_info['by_resource'].items():
                f.write(f'  {resource}: {count}\n')
            
            f.write('\nViolations by Activity:\n')
            for activity, count in self.violation_info['by_activity'].items():
    ...
    def run_full_analysis(self, max_traces: int = None, violation_rate: float = 0.05):
        ...
        Args:
            ...
            violation_rate: Percentage of events to modify as violations
        """
        print("Starting resource availability policy mining analysis...")
        ...
        # Detect violations using policy-aware approach
...
def main():
    ...
    print("Resource availability policy mining analysis completed!")
