...
"""
Policy Mining and Conformance Checking Framework (Optimized Version)
...
This version is optimized for memory usage and performance.
...
class PolicyMiningFramework:
    ...
    def preprocess_event_log(self) -> pd.DataFrame:
        ...
        if 'duration' not in self.log_df.columns:
            ...
            # Drop intermediate column to save memory
            self.log_df.drop('next_timestamp', axis=1, inplace=True)
    ...
    def define_policy_thresholds(self, method: str = 'percentile', percentile: float = 95) -> Dict[str, float]:
        ...
        if method == 'percentile':
            ...
            for resource in self.log_df['org:resource'].unique():
                durations = self.log_df[self.log_df['org:resource'] == resource]['duration'].dropna()
                self.policy_thresholds[resource] = np.percentile(durations, percentile)
                # Free memory
                del durations
                gc.collect()
            ...
            for resource in self.log_df['org:resource'].unique():
                group = self.log_df[self.log_df['org:resource'] == resource]
            ...
            for resource in self.log_df['org:resource'].unique():
                durations = self.log_df[self.log_df['org:resource'] == resource]['duration'].dropna()
                self.policy_thresholds[resource] = np.percentile(durations, 90)
                # Free memory
                del durations
                gc.collect()
    ...
    def detect_policy_violations(self) -> pd.DataFrame:
        ...
        eventlog_thresholds = {}
        for resource in self.log_df['org:resource'].unique():
            durations = self.log_df[self.log_df['org:resource'] == resource]['duration'].values
    ...
    def calculate_edit_distance(self) -> pd.DataFrame:
        ...
        case_ids = self.log_df['case:concept:name'].unique()
        batch_size = 100
    ...
    def visualize_results(self):
        ...
        # Plot 1: Duration distribution by resource (sample for top resources)
        plt.subplot(2, 2, 1)
        
        # Get top 5 resources by event count
        top_resources = self.log_df['org:resource'].value_counts().head(5).index.tolist()
        ...
        plt.title('Duration Distribution by Top Resources', fontsize=14)
    ...
    def save_results(self):
        ...
        # Save the analysis results (sample to avoid memory issues)
        sample_size = min(10000, len(self.log_df))
        self.log_df.sample(sample_size).to_csv(os.path.join(self.output_dir, 'analysis_results_sample.csv'), index=False)
...
def main():
    ...
    # Run full analysis with a very small subset of traces for demonstration
    # Use a smaller number for testing, or None for the full dataset
    framework.run_full_analysis(max_traces=200)
